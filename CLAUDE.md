...
- **`scripts/build-index.mjs`** — parses `Sky Atlas.md`, emits `public/docs.json` (`Record<uuid, AtlasNode>`) and `public/search-index.json` (serialized lunr index). Full-content indexing is intentional — search quality over bundle size.
- **`scripts/build-glossary.mjs`** — finds all `Definitions` sections, collects direct `[Core]` children as terms, emits `public/glossary.json` keyed by lowercased term.
- **`scripts/build-addresses.mjs`** — fetches Sky chainlog, calls Etherscan `getsourcecode` per unique address (read-through disk cache at `.cache/etherscan/<chainid>/<addr>.json`), emits `public/addresses.json`.
- **`scripts/build-graph.mjs`** — pattern-driven relation extraction, emits `public/graph.json` (node graph) and `public/relations.json` (typed edges). See `.claude/skills/graph-atlas/SKILL.md` for the full relationship reference.
- **`scripts/build-history.mjs`** — walks git log of the atlas submodule, emits `public/history/<uuid>.json` per node.
- **`scripts/build-manifest.mjs`** — sha256 digest of every shipping artifact; `vite.config.ts` reads it at build time for integrity verification.
...
Detected at build time in `build-index.mjs`. Each node stores `addressRefs: string[]` pointing into the shared `addresses.json`.
...
- `roles`: `string[]` — flat multi-tag array from a closed vocabulary (`ROLE_VOCAB` in `build-index.mjs`).
...
- **Each build pass gets its own script** (`scripts/build-<thing>.mjs`) and its own `pnpm build:<thing>`. Don't add new passes to `build-index.mjs`.
...
`public/chain-state.json` exists but is populated by `scripts/fetch-snapshots.mjs`. The frontend reads it via `loadChainState()` and `AddressCard` displays values. What's deferred:
...
```
scripts/build-index.mjs             parse Sky Atlas.md → docs.json + search-index.json
scripts/build-glossary.mjs          extract Definitions → glossary.json
scripts/build-addresses.mjs         chainlog + Etherscan → addresses.json (cache at .cache/etherscan/)
scripts/build-graph.mjs             relation extraction → graph.json + relations.json
scripts/build-history.mjs           git log of atlas submodule → history/<uuid>.json
scripts/build-manifest.mjs          sha256 of artifacts → manifest.json
scripts/build-at.mjs                reproducible build at a pinned atlas commit
scripts/fetch-snapshots.mjs         viem multicall snapshots → chain-state.json
