...
const isICD = d => /instance configuration document/i.test(d.title) && !isICDLocation(d);
...
const DIRECTORY_RE = /^The documents? herein (define|contain|organize|govern|specify|describe|set|compose|hold)\b/i;
...
const stripSentence = (prefixRe, suffixRe = null) => (s) => {
...
function extractInstanceParams(icd) {
...
for (const icd of allDocs.filter(d => isICD(d) && d.doc_no.startsWith("A.6.1.1."))) {
  ...
  const params = extractInstanceParams(icd);
...
for (const d of allDocs.filter(d => isICD(d) && d.doc_no.startsWith("A.6.1.1."))) {
  ...
  const status = inScope ? instanceStatusFor(d, primRoot) : null;
