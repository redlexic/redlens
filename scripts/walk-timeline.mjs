...
const commits = execSync('git -C ' + ATLAS + ' log --format=%H', { encoding: 'utf8' }).trim().split('\n');
...
function runAndTest(sha) {
  ...
  const build = spawnSync('pnpm', ['build:at', sha], { cwd: ROOT, encoding: 'utf8', maxBuffer: 50*1024*1024 });
  fs.writeFileSync(log, (build.stdout||'') + (build.stderr||''));
  if (build.status !== 0) return { status: 'BUILD_FAILED', passed: 0, failed: 99 };
  const test = spawnSync('pnpm', ['test'], { cwd: ROOT, encoding: 'utf8', maxBuffer: 50*1024*1024 });
  const out = (test.stdout||'') + (test.stderr||'');
  fs.appendFileSync(log, '\n--- TEST ---\n' + out);
  const passM = out.match(/\bTests\b.*?(\d+) passed/);
  const failM = out.match(/\bTests\b.*?(\d+) failed/);
  return { status: 'OK', passed: passM ? parseInt(passM[1]) : 0, failed: failM ? parseInt(failM[1]) : 0 };
}
...
for (let i = 0; i <= 100 && i < TOTAL; i += 5) {
  ...
  fs.appendFileSync(RESULTS, 'MAIN ' + i + ' ' + short + ' ' + status + ' ' + passed + ' ' + failed + ' | ' + msg + '\n');

  if (failed > 0 && prev_fail === 0 && i > 0) {
    console.log('  *** BREAK between commits ' + prev_i + '...' + i + ' -- bisecting ***');
    let found = false;
    for (let j = prev_i + 1; j < i; j++) {
      const bsha = commits[j]; const bshort = bsha.slice(0,12); const bmsg = getMsg(bsha);
      process.stdout.write('  bisect[' + j + '] ' + bshort + '  ' + bmsg + '\n');
      const r = runAndTest(bsha);
      process.stdout.write('           ' + r.status + '  pass=' + r.passed + '   fail=' + r.failed + '\n');
      fs.appendFileSync(RESULTS, 'BISECT ' + j + ' ' + bshort + ' ' + r.status + ' ' + r.passed + ' ' + r.failed + ' | ' + bmsg + '\n');
      if (r.failed > 0 && !found) {
        console.log('  >>> BREAKS AT [' + j + '] ' + bshort + ': ' + bmsg + ' <<<');
        fs.appendFileSync(RESULTS, 'BREAK ' + j + ' ' + bshort + ' ' + r.status + ' ' + r.passed + ' | ' + bmsg + '\n');
        found = true;
      }
    }
  }
