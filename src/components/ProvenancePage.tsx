...
const STAGES: Stage[] = [
  {
    ...
    outputs: [
      { name: "public/docs.json", note: "uuid → { doc_no, title, type, depth, parentId, content }" },
      { name: "public/search-index.json", note: "serialized lunr index, full-content" },
      { name: "public/addresses.merged.json", note: "intermediate; gitignored; handed to build:addresses" },
    ],
    powers: [
      "Full-text search (lunr in a Web Worker)",
      "Document viewing — every atlas node page, breadcrumbs, and UUID-to-UUID linking",
    ],
    verify: "Heading regex drives node extraction. Count: grep -c '<!-- UUID:' vendor/next-gen-atlas/Sky\\ Atlas/Sky\\ Atlas.md. Submodule SHA is pinned — see footer → atlas link.",
  },
  {
    ...
    outputs: [
      { name: "public/addresses.json", note: "labels, roles, aliases, expectedTokens. No ABIs ship to the frontend." },
    ...
    verify: "Cache is checked in — diff .cache/etherscan/ between commits to see what changed. Label priority: chainlog > atlas entityLabel > etherscan ContractName.",
  },
  {
    ...
    outputs: [
      { name: "public/chain-state.json", note: "{ generatedAt, block, values }. BigInts serialized as decimal strings." },
    ],
    powers: [
    ...
    verify: "Footer shows block number + generatedAt. Every snapshot is a git commit — git blame surfaces drift at the line level.",
  },
  {
    label: "atlas history",
    ...
    outputs: [
      { name: "public/history/<uuid>.json", note: "per-node change list; Atlas Edit Proposal PRs matched to nodes via bullet-title overlap" },
    ],
    powers: [
    ...
    verify: "Each entry links to the upstream sky-ecosystem commit and PR. Re-run the script to rebuild from scratch; output is deterministic given the same git + gh state.",
  },
  {
    label: "build graph",
...
const FRONTEND: { where: string; reads: string }[] = [
  { where: "src/lib/docs.ts", reads: "docs.json — module-level cache; every node view reads from here" },
  ...
  return (
    <div className="flex-1 overflow-y-auto px-6 py-8">
      <div className="max-w-3xl mx-auto">
        ...
        <h1 className="text-xl font-semibold mb-4" style={{ color: "var(--tan)" }}>Data flow &amp; provenance</h1>
        <p className="text-sm mb-8" style={{ color: "var(--tan-2)" }}>
          Everything in this app derives from a single source: <span className="mono">Sky&nbsp;Atlas.md</span>,
          pinned via git submodule at <span className="mono">vendor/next-gen-atlas/</span>. The exact commit is shown
          in the footer and links back to <span className="mono">sky-ecosystem/next-gen-atlas</span>. Every downstream
          artifact either lives in git or is reproducible from a single <span className="mono">pnpm build</span>.
        </p>
        ...
        {STAGES.map((s, i) => (
          <section key={s.label} className="mb-8">
            <div className="flex items-baseline gap-3 mb-2">
              ...
              <span className="mono text-sm" style={{ color: "var(--accent)" }}>{s.script}</span>
            </div>
            <div className="pl-7 space-y-2">
                ...
                <div className="space-y-1 flex-1">
                  {s.outputs.map(o => (
                    <div key={o.name} className="text-xs">
                      <span className="mono" style={{ color: "var(--tan)" }}>{o.name}</span>
              ...
              <div className="flex gap-4">
                ...
                <ul className="space-y-1 flex-1 text-xs" style={{ color: "var(--tan-2)" }}>
                  {s.powers.map(p => <li key={p}>· {p}</li>)}
                </ul>
              </div>
              <div className="flex gap-4">
                <span className="mono text-xs text-tan-3 w-14 shrink-0">verify</span>
                <p className="text-xs flex-1" style={{ color: "var(--tan-2)" }}>{s.verify}</p>
              </div>
            </div>
          </section>
        ))}

        <h2 className="text-sm font-semibold mb-3 mt-10" style={{ color: "var(--tan)" }}>What the frontend does with it</h2>
        <div className="space-y-1 mb-8">
          {FRONTEND.map(f => (
            <div key={f.where} className="flex gap-4 text-xs">
              <span className="mono shrink-0 w-64" style={{ color: "var(--accent)" }}>{f.where}</span>
              <span style={{ color: "var(--tan-2)" }}>{f.reads}</span>
            </div>
          ))}
        </div>

        <h2 className="text-sm font-semibold mb-2" style={{ color: "var(--tan)" }}>End-to-end trace</h2>
        <p className="text-xs mb-8" style={{ color: "var(--tan-2)" }}>
          For any claim on this site the chain is: atlas commit (footer) → <span className="mono">Sky&nbsp;Atlas.md</span> heading
          with embedded UUID → parsed node in <span className="mono">docs.json</span> → rendered by <span className="mono">NodeContent</span>.
          Addresses additionally trace through <span className="mono">addresses.json</span> + the committed Etherscan cache.
...
function Row({ label, items }: { label: string; items: string[] }) {
  return (
    <div className="flex gap-4">
      ...
      <div className="space-y-1 flex-1">
        {items.map(i => (
          <div key={i} className="mono text-xs" style={{ color: "var(--tan-2)" }}>{i}</div>
        ))}
      </div>
    </div>
  );
