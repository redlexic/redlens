...
function ChainBox({ node, isCurrent, onActor, onNavigate }: {
  node: ChainNode; isCurrent: boolean;
  onActor: (slug: string) => void; onNavigate: (id: string) => void;
}) {
...
export function ActorChain({ chain, currentSlug, onActor, onNavigate }: Props) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-1 gap-y-2 text-xs mono py-3 border-b border-[var(--border)]"
      style={{ color: "var(--tan-3)" }}>
      {prime
        ? <ChainBox node={prime} isCurrent={isCurrent(prime)} onActor={onActor} onNavigate={onNavigate} />
        : <span className="text-tan-3">—</span>}

      <span>/</span>

      {executors.length > 0
        ? executors.map(ex => <ChainBox key={ex.id} node={ex} isCurrent={isCurrent(ex)} onActor={onActor} onNavigate={onNavigate} />)
        : <span className="text-tan-3">—</span>}
      ...
      <div className="flex flex-wrap gap-1">
        {facilitators.length > 0
          ? facilitators.map(f => <ChainBox key={f.id} node={f} isCurrent={isCurrent(f)} onActor={onActor} onNavigate={onNavigate} />)
          : <span className="text-tan-3">—</span>}
        {govops.map(g => <ChainBox key={g.id} node={g} isCurrent={isCurrent(g)} onActor={onActor} onNavigate={onNavigate} />)}
      </div>
    </div>
  );
}
