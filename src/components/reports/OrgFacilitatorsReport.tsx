...
import { CATEGORY_LABELS, type OFResponsibility, deriveResponsibilities } from "../../lib/facilitatorResponsibilities";
...
export function OFReport({ onNavigate }: { onNavigate: (id: string) => void }) {
  ...
  const chains = useMemo<Map<string, AgentChain>>(() => {
    ...
    for (const prime of primes) {
      ...
      const facEdge = members.find(e => parseMeta<{ role?: string }>(e.m)?.role?.includes("facilitator"));
